...
from constants import NavigationsConstants as NC, URLS


@pytest.mark.parametrize("page_url, ex_page_name", 
                         NC.AUTHED_PAGES, 
                         scope="class")
class TestNavigationStructureAuthed():
    """
    ...
    def page(self, browser, new_user, page_url, ex_page_name):
        return setup_page(Navigation, browser, page_url)
    ...
    def test_cs50logo_is_visible(self, page):
        ...
        assert page.get_cs50_logo() == NC.CS50_LOGO, (
            f"Page Logo in the navigation bar (sum of spans inside the default page link) doesnt spell {NC.CS50_LOGO}"
            )
...
@pytest.mark.parametrize("page_url, ex_page_name", 
                         NC.UNAUTHED_PAGES,
                         scope="class")
class TestNavigationStructureUnauthed():
    """
...
@pytest.mark.parametrize("page_url, ex_page_name", 
                         NC.AUTHED_PAGES)
class TestNavigationRoutesAuthed():
    """
...
class TestSessionEnders():
    ...
    @pytest.fixture(autouse=True, scope="function")
    def page(self, browser, new_user, page_url, ex_page_name):
        yield setup_page(Navigation, browser, page_url)
...
@pytest.mark.parametrize("page_url, ex_page_name", 
                         NC.UNAUTHED_PAGES,
                         scope="class")
class TestNavigationRoutesUnauthed():
    """
